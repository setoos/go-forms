/*
  # Fix Quiz Analytics Function Column References
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(AVG(NULLIF(qa.score, 0)), 0) as average_score,
    COALESCE(
      (SUM(CASE WHEN qa.score >= 70 THEN 1 ELSE 0 END)::float / 
      NULLIF(SUM(CASE WHEN qa.score IS NOT NULL THEN 1 ELSE 0 END), 0)),
      0
    ) as pass_rate,
    COALESCE(
      (SUM(CASE WHEN qa.completed_at IS NOT NULL THEN 1 ELSE 0 END)::float / 
      NULLIF(COUNT(*), 0)),
      0
    ) as completion_rate,
    COALESCE(
      AVG(
        CASE 
          WHEN qa.completed_at IS NOT NULL THEN 
            EXTRACT(EPOCH FROM (qa.completed_at - qa.started_at))
          ELSE NULL 
        END
      ),
      0
