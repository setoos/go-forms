...
-- Create RLS policies with updated names to avoid conflicts
CREATE POLICY "quiz_analytics_view_policy"
  ON quiz_analytics
  FOR SELECT
  TO public
  USING (EXISTS (
...
CREATE POLICY "question_analytics_view_policy"
...
CREATE POLICY "quiz_trends_view_policy"
  ON quiz_trends
  FOR SELECT
  TO public
  USING (EXISTS (
...
CREATE POLICY "user_progress_view_policy"
  ON user_progress
  FOR SELECT
  TO authenticated
