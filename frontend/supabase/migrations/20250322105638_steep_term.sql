/*
  ...
  1. Changes
    ...
    - Fix quiz responses policy to work without quiz_id initially

  2. Security
    - Enable RLS on all tables
    - Add policies for quiz access
    - Add policies for question access
    - Add policies for option access
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'quiz_responses' AND policyname = 'Users can read own responses'
  ) THEN
    CREATE POLICY "Users can read own responses"
    ON quiz_responses FOR SELECT TO authenticated
    USING (auth.uid() IN (SELECT auth.uid() FROM users WHERE users.email = quiz_responses.email));
  END IF;
