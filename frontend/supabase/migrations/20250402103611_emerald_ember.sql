...
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "Users can view all templates" ON report_templates;
...
BEGIN
  SELECT COUNT(*) INTO standard_count FROM report_templates WHERE name = 'Standard Report';
  SELECT COUNT(*) INTO detailed_count FROM report_templates WHERE name = 'Detailed Analysis Report';
  
  IF standard_count = 0 THEN
    INSERT INTO report_templates (name, content, created_by, is_default)
    VALUES (
      'Standard Report',
...
DO $$
BEGIN
  -- This is a placeholder as we can't directly create storage buckets in SQL
  -- You'll need to create the 'template-images' bucket in the Supabase dashboard
