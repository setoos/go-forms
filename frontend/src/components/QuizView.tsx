...
import { generateQuizShare, validateQuizAccess } from '../lib/quiz';
...
export default function QuizView() {
  ...
  async function loadQuiz() {
    try {
      ...
      setShareUrl(`${window.location.origin}/quiz/${quizData.share_id || quizData.id}`);
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-xl p-8">
        <div className="flex items-start justify-between mb-8">
          ...
          <button
            ...
            className="flex items-center px-4 py-2 text-purple-600 hover:text-purple-800 border border-purple-600 rounded-lg hover:bg-purple-50 transition-colors"
          >
        ...
        {showShareDialog && (
          <div className="mb-8 p-6 bg-gray-50 rounded-lg">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900">Share Quiz</h2>
              {getAccessTypeIcon()}
            </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    readOnly
                    value={shareUrl}
                    className="flex-1 px-3 py-2 bg-white border border-gray-300 rounded-md text-gray-600"
                  />
                  <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          {quiz.time_limit && (
            <div className="p-4 bg-gray-50 rounded-lg">
              ...
              <p className="text-2xl font-bold text-gray-900">{quiz.time_limit} minutes</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
          {questions.slice(0, 3).map((question, index) => (
            <div key={question.id} className="border-b border-gray-200 pb-6 last:border-0">
              ...
              <div className="space-y-3">
                {question.options && question.options.slice(0, 3).map((option, optionIndex) => (
                  <div
                    key={option.id}
                    className="flex items-center p-3 bg-gray-50 rounded-lg"
                  >
                    <span className="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full bg-white text-gray-600 mr-3">
          ...
          {questions.length > 3 && (
            <div className="text-center text-gray-500 py-4 border-t border-gray-200">
              {questions.length - 3} more questions not shown
            </div>
          )}
        </div>
        ...
        <div className="mt-8 flex justify-center">
          <button
            onClick={() => navigate(`/quiz/${quiz.share_id || quiz.id}`)}
