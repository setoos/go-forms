  ...
  const handleImageUpload = async () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        setCustomFeedback(
          (prev) =>
            `${prev}<p><img src="${publicURL.publicUrl}" alt="Uploaded image" style="max-width: 100%; height: auto;" /></p>`
        );
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    {quizTitle ? `${quizTitle} - ` : ""}Detailed analysis of
                    your performance
                  </p>
                </div>
              </div>
                      ...
                      datasets: [
                        {
                          ...
                          data: [
                            Object.values(answers).filter(
                              (s) => Number(s) >= 0 && Number(s) <= 2
                            ).length,
                            Object.values(answers).filter(
                              (s) => Number(s) >= 3 && Number(s) <= 4
                            ).length,
                            Object.values(answers).filter(
                              (s) => Number(s) >= 5 && Number(s) <= 6
                            ).length,
                            Object.values(answers).filter(
                              (s) => Number(s) >= 7 && Number(s) <= 8
                            ).length,
                            Object.values(answers).filter(
                              (s) => Number(s) >= 9 && Number(s) <= 10
                            ).length,
                          ],
                          backgroundColor: [
                            "rgba(239, 68, 68, 0.7)",
