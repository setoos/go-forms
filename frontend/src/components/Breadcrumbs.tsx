...
const breadcrumbConfigs: Record<string, BreadcrumbConfig[]> = {
  '/admin/quizzes': [
    ...
    { path: '/admin/quizzes', label: 'My GoForms' }
  ],
  '/admin/quizzes/new': [
    { path: '/', label: 'Dashboard', icon: <Home className="h-4 w-4" /> },
    { path: '/admin/quizzes', label: 'My GoForms' },
    { path: '/admin/quizzes/new', label: 'Create GoForm' }
  ],
  '/admin/analytics': [
    { path: '/', label: 'Dashboard', icon: <Home className="h-4 w-4" /> },
  ...
  '/admin/submissions': [
    { path: '/', label: 'Dashboard', icon: <Home className="h-4 w-4" /> },
    { path: '/admin/submissions', label: 'GoForm Submissions', icon: <Inbox className="h-4 w-4" /> }
  ],
  '/forms/templates': [
    { path: '/', label: 'Dashboard', icon: <Home className="h-4 w-4" /> },
  ...
  '/forms/categories/academic': [
    { path: '/', label: 'Dashboard', icon: <Home className="h-4 w-4" /> },
    { path: '/forms/templates', label: 'Templates Library' },
    { path: '/forms/categories/academic', label: 'Academic GoForm Formats' }
  ],
  ...
  const getMatchingConfig = () => {
    ...
    if (location.pathname.startsWith('/admin/quizzes/') && location.pathname !== '/admin/quizzes/new') {
      return [
        ...
        { path: '/admin/quizzes', label: 'My GoForms' },
        { path: location.pathname, label: 'Edit GoForm' }
      ];
    }
    ...
    if (location.pathname.startsWith('/admin/quizzes/') && location.pathname.includes('/analytics')) {
      return [
        { path: '/', label: 'Home', icon: <Home className="h-4 w-4" /> },
        { path: '/admin/quizzes', label: 'My GoForms' },
        { path: location.pathname, label: 'GoForm Analytics' }
      ];
    }
    ...
    if (location.pathname.startsWith('/admin/submissions/')) {
      return [
        { path: '/', label: 'Home', icon: <Home className="h-4 w-4" /> },
        { path: '/admin/submissions', label: 'GoForm Submissions' },
